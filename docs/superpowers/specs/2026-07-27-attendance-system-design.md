...
- **`/teacher` 儀表板新增兩處入口**（比照現有「弈廳管理」清單模式，讓老師不用先去總覽頁選日期）：
  - 頂部捷徑卡片列新增「點名」，連到 `/teacher/attendance`（點名總覽，可切換日期）
  - 頁面下方新增「今日點名」區塊，直接列出該老師今天負責的場次（班級／一對一補課／弈廳／活動，固定抓「今天」），每列可直接點入開名單
