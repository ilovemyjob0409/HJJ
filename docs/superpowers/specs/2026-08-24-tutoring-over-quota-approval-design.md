...
- 超額預約送審成立 → 推播行政「需要審核」（符合 2026-08-20「行政只收需審核通知」慣例）；老師只在預約確定成立時收到通知——一般預約建立當下、超額預約核准當下（駁回不通知）。
- 核准／駁回 → 推播該學生（`pushToUser`，url 指向 `/student/tutoring`）。
- 推播失敗只記 log，不影響主流程（沿用既有慣例）。

## 邊界情況

- **到了上課日還沒審**（2026-08-24 使用者定案）：待審預約**不會**出現在老師點名名單與掃碼機——行政核准前不能點名。學生若已到場，行政收到審核推播後在後台核准，名單即出現、即可點名；核准前的既有防呆（同日重複、佔名額）不變。配套：建立待審預約時**不**通知時段老師（老師只收確定成立的預約通知），核准當下才補發老師通知；駁回不通知老師（與行政取消慣例一致）。
