...
import { getGoHallRoster, saveGoHallAttendance, clearGoHallAttendance } from '@/lib/services/attendanceService';
...
export async function DELETE(req: NextRequest, { params }: { params: { sessionId: string } }) {
  ...
  const body = await req.json();
  await clearGoHallAttendance(params.sessionId, body.clear ?? []);
  return NextResponse.json({ success: true });
}
