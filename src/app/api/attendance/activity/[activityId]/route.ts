...
import { getActivityRoster, saveActivityAttendance, clearActivityAttendance } from '@/lib/services/attendanceService';
...
export async function DELETE(req: NextRequest, { params }: { params: { activityId: string } }) {
  ...
  const body = await req.json();
  if (!body.date) return NextResponse.json({ error: 'date required' }, { status: 400 });
  await clearActivityAttendance(params.activityId, new Date(body.date), body.clear ?? []);
  return NextResponse.json({ success: true });
}
